...
export default MoviePageimport React, { useEffect, useState } from 'react';
import { useParams } from 'react-router-dom';
import { Clock, Star, Award, Film, User, Bookmark, BookmarkCheck } from 'lucide-react';
import VideoPlayer from '../components/player/VideoPlayer';
import MovieCarousel from '../components/ui/MovieCarousel';
...
const MoviePage: React.FC = () => {
  ...
  return (
    <div className="pt-16 bg-black text-white relative">
      ...
      <div className="relative z-10 max-w-screen-2xl mx-auto px-6 xl:px-12">
        <div className="mb-10">
          ...
          <div className="flex flex-wrap items-center gap-6 text-lg text-gray-300 mb-8">
            ...
            <button
              onClick={handleBookmark}
              className={`flex items-center gap-2 px-4 py-2 rounded-full border text-sm md:text-base font-medium transition ${
                isBookmarked
                  ? 'border-red-500 text-red-500'
                  : 'border-gray-700 text-gray-400 hover:border-gray-500'
              }`}
          ...
          <div className="w-full xl:w-1/3">
            <div className="bg-gray-900 rounded-2xl p-8 shadow-lg">
              ...
              <div className="space-y-6">
                {movie.director && (
                  <div className="flex items-start">
                    <User className="text-red-500 mr-4 mt-1" size={20} />
                    <div>
                      <p className="text-gray-400 text-sm">Director</p>
                      <p className="text-white text-base">{movie.director}</p>
                    </div>
                  </div>
                )}
                ...
                {movie.actors && (
                  <div className="flex items-start">
                    ...
                    <div>
                      <p className="text-gray-400 text-sm">Cast</p>
                      <p className="text-white text-base">{movie.actors}</p>
                    </div>
                  </div>
                )}
                ...
                {movie.genre && (
                  <div className="flex items-start">
                    <Film className="text-red-500 mr-4 mt-1" size={20} />
                    <div>
                      <p className="text-gray-400 text-sm">Genre</p>
                      <p className="text-white text-base">{movie.genre}</p>
                    </div>
                  </div>
                )}
                ...
                {movie.awards && movie.awards !== 'N/A' && (
                  <div className="flex items-start">
                    <Award className="text-red-500 mr-4 mt-1" size={20} />
                    <div>
                      <p className="text-gray-400 text-sm">Awards</p>
                      <p className="text-white text-base">{movie.awards}</p>
                    </div>
                  </div>
                )}
              </div>
